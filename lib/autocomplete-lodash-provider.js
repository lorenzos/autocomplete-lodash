...
export default class AutocompleteLodashProvider {
  ...
  static buildMethodCompletion(method) {
    ...
    return {
      name,
      snippet: this.getPrefix().prefix + `${name}(${args.map((arg, i) => `\$\{${i+1}:${arg}\}`).join(', ')})`,
      type: 'function',
      description,
  ...
  static getLodashMethod(request) {
    ...
    const matches = this.getPrefix().pattern.exec(line);
  ...
  static getLodashMethodsCompletions(prefix) {
    ...
    return completions.map(method => {
      method.replacementPrefix = this.getPrefix().prefix + prefix;
  ...
  /**
   * Returns the Lodash prefix type selected by the user in Settings
   * @return {Object} One of the objects in lodashPrefixes
   */
  static getPrefix() {
    return lodashPrefixes[atom.config.get('autocomplete-lodash.prefixType')];
  }
